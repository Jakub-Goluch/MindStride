...
def update_plot(new_segment_idx):
    ...
    for freq_band in range(n_freq_bands):
        ...
        combined_transformed_data.append(transformed_data)
    ...
    combined_transformed_data = np.concatenate(combined_transformed_data, axis=1)
...
for i in range(n_segments - 1):  
    plt.close('all')  
    combined_transformed_data = update_plot(i)
    print(f"Combined transformed data for segment {i}:\n", combined_transformed_data)
    plt.pause(1)  
