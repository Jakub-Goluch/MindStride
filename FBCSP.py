...
def train_csp_models(data, labels, freq_bands, class_standard, n_components, update_existing=False, model_file=None):
    ...
    for class_idx, class_name in class_standard.items():
        ...
        for band_idx, (fmin, fmax) in enumerate(freq_bands):
            ...
            if csp_models[class_name][freq_name] is None:
                csp_models[class_name][freq_name] = CSP(n_components=n_components, reg=None, log=True, norm_trace=False)
            try:
...
def process_folder(folder_path, model_file, n_components, update_existing):
    for root, dirs, files in os.walk(folder_path):
        for file in files:
            if file.endswith('.csv'):
                ...
                csp_models = train_csp_models(data, labels, freq_bands, classStandard, n_components, update_existing, model_file)
...
def main():
    ...
    parser.add_argument('--n_components', type=int, default=2, help="Number of CSP components to use.")
    parser.add_argument('--update_existing', action='store_true', help="Flag to update existing models.")
    args = parser.parse_args()

    process_folder(args.data_dir, args.model_file, args.n_components, args.update_existing)
